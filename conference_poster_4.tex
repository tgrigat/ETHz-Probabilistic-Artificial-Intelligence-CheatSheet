...
\textbf{Definition: Gaussian (Normal) distribution}
{\footnotesize
$$
...
\begin{itemize}\compresslist
    \item $D_{KL}(p||q)$(backward) : mode averaging 
...
$$
$$
P(y_* \mid X,Y,x_*) = \mathcal{N}\!\left(\bar{\mu}^\top x_*,\; x_*^\top \bar{\Sigma}\, x_* + \sigma_n^2\right)
$$
$$\vspace{-0.2cm}
\bar{\mu} = \frac{1}{\sigma_n^2}\bar{\Sigma} X^\top Y \quad
\bar{\Sigma} = \left(\frac{1}{\sigma_n^2}X^\top X + \frac{1}{\sigma_p^2}I\right)^{-1}
$$
...
\vspace{-0.2cm}
\begin{itemize}\compresslist
    \item posterior is not gaussian and not closed
...
\\
&P(f|X,Y, x^*)\sim GP(f;\mu',k')
\\
\mu'(x^*) &= \mu(x^*) + K_{x^*X}(K_{XX}+\sigma_n^2I)^{-1}Y
\\
k'(x^*) &= K_{x^*x^*} - K_{x^*X} (K_{XX}+\sigma_n^2I)^{-1}K_{x^*X}^\top
\end{aligned}
$$
...
\underline{\textbf{Kernel properties}}
\vspace{-0.2cm}
\begin{itemize}\compresslist
    \item \textbf{Stationary}: translation invariant, \(\;k(x,x')=k(x-x')\;\).
    \item \textbf{Isotropic/Stationary}: depends only on the distance, \(\;k(x,x')=k(\|x-x'\|)\;\).
    \item \textbf{Isotropic}: rotation invariant, \(\;k(Rx,Rx')=k(x,x')\;\).

\end{itemize}

\underline{\textbf{Kernel conditions}}
\vspace{-0.2cm}
$$
K_{ij}=k(x_i,x_j)
$$
\begin{itemize}\compresslist
    ...
    \item positive semi-definiteness: for any finite set $\{x_i\}_{i=1}^n$ and any $z\in\mathbb{R}^n$, \; $z^\top K z \ge 0$ \quad (ensures variances $\ge 0$) 
\end{itemize}
\underline{\textbf{Kernel composition rules}}
\vspace{-0.2cm}
\begin{itemize}\compresslist
    \item $k_1,k_2: X\times X\to\mathbb R$
    \item $k(x,x') = k_1(x,x') + k_2(x,x')$
    \item $k(x,x') = c\,k_1(x,x'),\quad c>0$
    \item $k(x,x') = k_1(x,x')\,k_2(x,x')$
    \item $k(x,x') = f\big(k_1(x,x')\big)$
    \item $k(x,x') = \sum_{i=1}^n \phi_i(x)\,\phi_i(x')$ (feature maps are valid)
\end{itemize}

\vspace{-0.2cm}
\underline{\textbf{Bivariate covariance}}
\vspace{-0.2cm}
$$
\operatorname{Cov}\!\begin{pmatrix}X\\[2pt]Y\end{pmatrix}
=
\begin{bmatrix}
\operatorname{Var}(X) & \operatorname{Cov}(X,Y)\\[4pt]
\operatorname{Cov}(X,Y) & \operatorname{Var}(Y)
\end{bmatrix}
$$
\noindent where \(\operatorname{Cov}(X,Y)=k(x,y)\) (kernel function)

\colorbox[HTML]{CCFFFF}{\makebox[\textwidth-2\fboxsep][l]{\bf - Hyperparameter optimization}}
\vspace{-0.3cm}
$$
\log p(y\mid X,\theta)= -\tfrac{1}{2}y^\top K_y^{-1}y
-\tfrac{1}{2}\log|K_y|-\tfrac{n}{2}\log(2\pi),\quad
$$
$$
K_y\coloneqq K_{f,\theta}+\sigma_n^2 I,\ \alpha\coloneqq K_y^{-1}y
$$
$$
\frac{\partial}{\partial\theta_j}\log p(y\mid X,\theta)
= \tfrac{1}{2}\operatorname{tr}\!\Big((\alpha\alpha^\top - K_y^{-1})\frac{\partial K_y}{\partial\theta_j}\Big)
$$
...
\underline{\textbf{Prior / Likelihood}}
\vspace{-0.2cm}
$$
p(\theta)\,,\qquad
p(\mathcal D\mid\theta)=\prod_{i=1}^n p(y_i\mid x_i,\theta)\quad(\text{i.i.d.})
$$
\underline{\textbf{Bayesian update}}
\vspace{-0.3cm}
$$
...
\underline{\textbf{Posterior predictive (prediction)}}
\vspace{-0.3cm}
$$
p(y_*\mid x_*,\mathcal D)=\int p(y_*\mid x_*,\theta)\,p(\theta\mid\mathcal D)\,d\theta
$$

\textbf{Probability laws(continuous case)}
\vspace{-0.2cm}
$$
Probability: P(A)=\int P(A\mid B=b)\,p_B(b)\,db
$$
$$
Expectation: \mathbb{E}[X]=\mathbb{E}\big[\mathbb{E}[X\mid Y]\big]
$$
$$
Var: \operatorname{Var}(X)=\mathbb{E}\big[\operatorname{Var}(X\mid Y)\big]+\operatorname{Var}\big(\mathbb{E}[X\mid Y]\big)
$$
}
%------------------------------------------------
...
\underline{\textbf{Laplacian approximation $q(\theta)\sim \mathcal{N}(\hat\theta, \Lambda^{-1})~~$}}
\vspace{-0.3cm}
$$
...
$$\vspace{-0.3cm}
\begin{aligned}
I(s) &= H(f) - H(f|y_s) = \frac{1}{2}log|I+\sigma_n^{-2}K_s|
\\
